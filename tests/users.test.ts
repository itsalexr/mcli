...
describe('fetchUsers', () => {
  it('returns users list', async () => {
    ...
    const result = await fetchUsers(mock.client, { limit: 200 });
  ...
  it('passes limit variable', async () => {
    const mock = createMockClient();
    mock.setResponse(USERS_RESPONSE);
    await fetchUsers(mock.client, { limit: 50 });
  ...
  it('propagates GraphQL errors', async () => {
    ...
    await expect(fetchUsers(mock.client, { limit: 200 })).rejects.toThrow('Unauthorized');
  });
});
